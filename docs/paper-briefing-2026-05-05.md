...
Most other paper-vs-FORTRAN choices on the seal example are biologically dormant on the cell-count metric: removing them from `LEGACY_FORTRAN` produces the same 60-cell plateau. This includes `eq17_inh_source`, `eq18_sec_source`, `eq5_apply_to`, `knot_daughter_di`, `laplacian` (the seal-example single-field disentanglement was run before the in-plane fortran_margins operator was wired in B3 (2026-05-05), so this row's seal-example metrics use length-weighted in both anchors; a B3-confirmation run at `experiments/discretisation-study/seal-fortran-margins-2026-05-05/` shows the LEGACY_FORTRAN seal output is byte-identical between length_weighted and fortran_margins at every save, so the seal disentanglement's laplacian row is unaffected by the operator change), `lattice_orientation`, `rep_neighbour_set`, `update_order`, and `border_definition`.
...
The `laplacian` row was a structural no-op in the prior cusp-forming disentanglement: `Discretisation.laplacian='fortran_margins'` raised `NotImplementedError`, and the runner auto-overrode it to `length_weighted` for both anchors. Path B v2 B3 (2026-05-05) implements the operator at `silicoshark/mesh.py:fortran_margins_laplacian` as the in-plane contact-area-weighted scheme — neighbours sorted angularly around the cell centre, margin midpoints at edge midpoints, `pes[k]` = perimeter-segment length, `area_p[k]` = cross-product triangle area, all normalised per the FORTRAN's `sum_a = sum(pes) + 2 * sum(area_p)`. The substrate sink, vertical-z-flux, and substrate-edge layer of the FORTRAN's full `apply_diffusion` are NOT modelled — that is a separate 5–7-day sub-project (the in-plane operator was 1.5 days). The full single-field disentanglement was re-run with the new operator at `experiments/discretisation-study/single-field-cusp-forming-fortran-margins-2026-05-05/`: 27 of the 28 rows are byte-identical to the original, with the only change at `PATH_B_DEFAULT_plus_laplacian` (cusp_width 0.6927900 → 0.2947925, −57 %; y-envelope halves). `LEGACY_FORTRAN_minus_laplacian` is byte-identical to the LEGACY_FORTRAN anchor at every save (SHA-256 `3947760b3bc1`). The row is no longer a structural no-op; under LEGACY_FORTRAN's saturated regime the laplacian operator's choice is empirically null, while under PATH_B_DEFAULT's settings the weighting is measurable. Findings doc: `docs/findings/2026-05-05-path-b-v2-fortran-margins-implementation.md`.
...
- **`fortran_margins` laplacian: in-plane operator wired; full byte-match deferred.** Path B v2 B3 (2026-05-05) implemented the in-plane contact-area-weighted operator at `silicoshark/mesh.py:fortran_margins_laplacian` (per-edge `pes`, per-cell `area_p`, FMA-LIMIT GUARD on `sum_a == 0`). The substrate sink (`-0.44 * pes * q`), vertical-z-layer flux, and substrate-edge layer of the FORTRAN's full `apply_diffusion` are NOT modelled — that byte-match against Path A's `coreop2d.py` is a separate 5–7-day sub-project (the in-plane operator was 1.5 days). The residual z_min divergence in the LEGACY_FORTRAN seal replication is now ruled out as caused by the in-plane operator: re-running LEGACY_FORTRAN on `wt-tribosphenic-2014.txt` is byte-identical between length_weighted and fortran_margins (SHA-256 `3947760b3bc1`). The z_min residual is a property of force propagation under `static_with_local_update`, not the laplacian operator. Whether the substrate sink would shift z_min remains an open phase-2 question.
...
The `LEGACY_FORTRAN` preset declares `laplacian='fortran_margins'`. Until B3 (Path B v2, 2026-05-05) that option raised NotImplementedError and the comparison-study runner auto-overrode it to `length_weighted` for both anchors. B3 wires the in-plane operator at `silicoshark/mesh.py:fortran_margins_laplacian`; the auto-override is no longer needed. The seal-example single-field disentanglement (Appendix C below) was run before B3 and uses length-weighted in both anchors. The cusp-forming re-run with the new operator (`experiments/discretisation-study/cusp-forming-fortran-margins-2026-05-05/`) shows that `LEGACY_FORTRAN` is byte-identical between the two operators on `wt-tribosphenic-2014.txt`, so the seal-disentanglement values would not be expected to change on the laplacian row. A direct seal-example test of this hypothesis is documented as follow-up work.
