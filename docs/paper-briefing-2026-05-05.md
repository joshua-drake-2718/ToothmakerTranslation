...
- **Cotangent (discrete Laplace–Beltrami) operator.** The textbook discrete Laplacian on triangulated surfaces, weighting each edge by `cot α + cot β` where α and β are the opposing angles in the two triangles sharing the edge (Meyer, Desbrun, Schröder, Barr, 2003). Closer to the paper's prose 'flux proportional to contact area' if 'contact area' is read as 'curvature-aware geodesic weighting'. No code-base in the lineage uses it.
...
- **Paper (Salazar-Ciudad and Jernvall 2010, eq. 17).** `partial[Inh] / partial t = [Act] - k_deg [Inh] + k_di nabla^2 [Inh]` for cells with `d_i >= k_int`. The source is the Act *concentration*.
...
What the LLM-assisted analysis cannot do:

1. **Validate that the simulator is dynamically correct.** Tests pin specific equations; they do not verify the underlying biology. The smoke tests assert that `LEGACY_FORTRAN` reproduces the FORTRAN goldens, but a hypothetical bug in `13.f90` would be silently inherited by `LEGACY_FORTRAN` without anyone noticing. The audit catalogues choice points; it does not prove the model itself is correct.

2. **Identify new choice points the analyst doesn't ask about.** The taxonomy is exhaustive of what was found, not provably exhaustive of all choice points. There may be a choice point in `update_cell_position` (or elsewhere in the FORTRAN) that no human or LLM has yet asked about, simply because nobody pointed at it. The methodology can scale only to the extent that the analyst keeps asking 'what else could be different?'

3. **Replace expert judgement on classification.** PaperAmbiguity vs PaperVsCodeTension is itself a reading. I have classified the eq. 14 typo as PaperVsCodeTension (the paper text and the code disagree, and one side is more credibly intended); a different analyst might classify it as PaperAmbiguity (the printed equation is mathematically well-defined; the code's choice of `[Inh]` is one of several defensible saturation forms). The classification matters because it changes the burden of justification. Where the analyst assigns a row matters; the LLM can produce a defensible classification but cannot adjudicate disputes about it.

A note on meta-methodology. The entire Path B v2 build was itself driven by a Claude Code orchestrator running fourteen phases (A1 through B5) as isolated agent subtasks, each with a self-contained prompt, paper citations, success criteria, and validation commands. This document is being written by one of those phase agents, named B5. The methodology paper has the option of including this self-reference as evidence that the LLM-assisted approach can handle multi-day software-construction projects, not just one-off audits. I am cautious about overclaiming here: each phase was scoped narrowly enough that an LLM could complete it without losing the plot, and the orchestrator's per-phase verification was non-trivial human design work (not done by an LLM). The proposition is that LLM-assisted analysis at the granularity of a single phase, run repeatedly under human-designed scaffolding, produces a thoroughness that human-only effort struggles to sustain over a multi-week project.

## Related work

I position the work briefly in three threads.

**Formal specification.** Hoare logic, separation logic, refinement types, deductive verification (Frama-C, Why3), and model-checking. These tools are mature for compilers, kernels, and small safety-critical systems where the specification can be made precise. They are impractical at the scale of computational biology, where the specification is inherently informal (a *Nature* letter and its supplementary materials) and the model itself is the object of inquiry. The relationship between this work and formal specification is one of complementarity: where formal methods can apply, they should; where they cannot — which includes most of science — LLM-assisted comparative analysis offers a partial bridge.

**Executable scientific papers.** Knuth's literate programming (1984), Jupyter and its predecessors (IPython, Mathematica, Maple), ReScience C (a journal devoted to reproducing computational results), and the broader 'reproducibility crisis' literature (Baker 2016 *Nature* survey, Munafò et al. 2017 *Nature Human Behaviour* manifesto). The aim of literate programming is to interleave prose and code so that the human reader can follow the implementation as a single document. The aim of ReScience and similar journals is to incentivise reproduction as its own publishable contribution. Both are valuable; neither directly addresses the per-line implementer-choice question. The proposed approach (LLM-assisted comparative discretisation studies) is positioned between formal spec and literate programming: not as rigorous as formal spec, but more auditable than narrative prose, and more empirically grounded than literate programming because the tests exercise each choice point directly.

**Model-implementation faithfulness in computational biology.** A growing literature on FAIR data principles (Wilkinson et al. 2016 *Scientific Data*), on reproducing other papers' models (e.g. the mc-stan and PyMC reimplementations of published Bayesian models), and on community challenges that probe model behaviour blindly (DREAM, CASP, ISCB). In computational biology specifically, ModelDB and the Physiome Project archive published models with metadata; the BioModels Repository (Le Novère et al. 2006 *Nucleic Acids Research*) does similar work for systems biology. None of these archive at the granularity of per-line implementer-choice; they store the model and its parameter set, not the audit trail of why each implementation choice was made.

The contribution of this work is to add the audit trail. The Discretisation dataclass, the audit document, and the comparison study together constitute a per-choice-point record that did not previously exist for this model. The methodology paper argues that such a record is itself a publishable scientific output, alongside whatever biological findings the comparison reveals. Whether the methodology generalises beyond developmental biology is a question for follow-up work; my conjecture is that it does, because the paper-vs-code gap is general and the LLM-assisted catalogue technique is domain-neutral.
